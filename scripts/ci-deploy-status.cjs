#!/usr/bin/env node
/**
 * Track local workbench website deploy progress with live log tail preview.
 *
 * Usage:
 *   npm run ci
 *   npm run ci -- --once
...
const LOG_TAIL_LINES = Number.parseInt(process.env.WORKBENCH_CI_LOG_LINES || "12", 10);
const ACTIVITY_LINES = Number.parseInt(process.env.WORKBENCH_CI_ACTIVITY_LINES || "8", 10);
...
function parseArgs(argv) {
  let once = false;
  let intervalMs = Number.parseInt(process.env.WORKBENCH_CI_INTERVAL || "2", 10) * 1000;
  ...
  npm run ci:infra                     full suherman.net production dashboard
  npm run deploy:website             deploy workbench.suherman.net

  WORKBENCH_CI_LOG_LINES=12          log tail lines (default 12)
  WORKBENCH_CI_INTERVAL=2            refresh interval in seconds (default 2)
`);
      process.exit(0);
    }
  }
...
function buildStatusBoxLines(
  ...
  if (active > 0) {
    ...
    lines.push(dim("Deploy: npm run deploy:website"));
  } else if (!everDeployed) {
    lines.push(yellow("No deployments yet."));
    lines.push(dim("Run: npm run deploy:website"));
  } else {
...
function printDashboard(state) {
  ...
  clearScreen();
  renderBox(
    "workbench.suherman.net local deployment",
    buildStatusBoxLines(
      state,
      rows,
      active,
