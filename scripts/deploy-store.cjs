...
function normalizeRepoStatus(rs) {
  ...
  if (isStuckQueued(rs)) {
    rs.status = "idle";
    if (!rs.lastError) {
      rs.lastError = "Deploy never started — run: npm run deploy:website";
    }
  }
}
...
function recordActivity(state, activity) {
  ...
  const max = Number.parseInt(process.env.WORKBENCH_ACTIVITY_MAX || "80", 10);
...
function upsertDeployment(state, deployment) {
  ...
  const max = Number.parseInt(process.env.WORKBENCH_DEPLOY_HISTORY_MAX || "100", 10);
